# functions.py
# Function Calling Helper Functions (local copy for syllabus quiz agents)
# Pairs with functions.R in 08_function_calling
# Tim Fraser
...
def agent_run(role, task, tools=None, output="text", model=DEFAULT_MODEL, num_predict=500):
    ...
    # Run the agent
    resp = agent(messages=messages, model=model, output=output, tools=tools, num_predict=num_predict)
    return resp
